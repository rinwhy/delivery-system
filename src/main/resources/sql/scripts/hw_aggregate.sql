...
select id as product_id, min(stock) as min_stock from products group by id limit 1;					-- shows what product has the lowest stock quantity
...
select orders.customer_id, sum(order_items.quantity) as total_quantity from order_items join orders on order_items.order_id = orders.id group by orders.customer_id;						-- shows the amount items ordered by each of the customers

select name as product_name, min(price) as price from products group by product_name having price <= 16;		-- products that cost less than or equal to $16
select name as product_name, max(price) as price from products group by product_name having price >= 16;		-- products that cost more than or equal to $16 
...
select id as delivery_id, status, count(*) from deliveries group by delivery_id having status = 'delivered';		-- the deliveries that have been completed     
